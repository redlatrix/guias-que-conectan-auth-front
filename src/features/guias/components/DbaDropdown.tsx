  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        disabled={isDisabled}
        onClick={() => setOpen((o) => !o)}
        className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base font-public bg-white text-left flex items-start justify-between gap-2 outline-none focus:border-copper focus:ring-1 focus:ring-copper transition disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed whitespace-normal"
      >
      ...
      {open && !isDisabled && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-80 overflow-y-auto">
          {dbas.length === 0 ? (
            <p className="text-base text-gray-400 px-4 py-4 font-public text-center">
            ...
            dbas.map((d) => (
              ...
              >
                <span className="block text-sm font-semibold text-copper font-public mb-0.5 uppercase tracking-wide">
                  {d.codigo_men}
                </span>
                <span className="block text-base text-gray-800 font-public leading-relaxed whitespace-normal break-words">
                  {d.enunciado_oficial}
                </span>
              </div>
            ))
